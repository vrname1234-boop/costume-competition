...
import { getSettings, isMaintenanceMode } from "../services/settings";
...
async function assertStudentAllowed(role: UserRow["role"]): Promise<void> {
  if (role !== "student") return;
  if (await isMaintenanceMode()) throw forbidden(MAINTENANCE_STUDENT);
}
...
async function consumeCode(
  ...
  if (!row)
    throw badRequest(
      "That code has expired or is not valid. Request a new one.",
    );
...
authRouter.post(
  ...
  rateLimit({
    ...
    key: emailKey,
    message:
      "A code was already sent to that address. Please wait before requesting another.",
  }),
...
authRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (user.status === "active")
      throw badRequest("That account is already set up. Please sign in.");
...
authRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (user.status === "pending") {
      throw forbidden(
        "Finish setting up your account using the code sent to your school email.",
      );
    }
...
authRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Only students hold an email address; staff reset through the Owner. So a
    // request here during maintenance is always a student trying to get in.
    if (await isMaintenanceMode()) throw forbidden(MAINTENANCE_STUDENT);
    const user = await queryOne<UserRow>(
...
authRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const account = await queryOne<UserRow>(
      `SELECT * FROM users WHERE email = $1 AND deleted_at IS NULL AND status = 'active'`,
      [email],
    );
    if (account) await assertStudentAllowed(account.role);
