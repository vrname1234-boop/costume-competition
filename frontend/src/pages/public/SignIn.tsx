...
export function SignIn() {
  ...
  const submit = (event: FormEvent) => {
    ...
    void signIn(identifier.trim(), password)
      .then((user) => {
        navigate(user.mustChangePassword ? '/change-password' : HOME_FOR_ROLE[user.role], {
          replace: true,
        });
      })
      .catch((err: unknown) => {
  ...
  return (
    <>
      <PageHeader title="Sign in" lead="Students sign in with their school email. Staff sign in with the username they were given." />
