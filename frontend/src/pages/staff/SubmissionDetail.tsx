...
export function StaffSubmissionDetail() {
  ...
  const selectedReason =
    reasons.find((reason) => reason.code === rejectCode) ?? null;
  ...
  return (
    <>
      <PageHeader
        ...
        lead={submission.studentEmail ?? undefined}
      />
      ...
      {submission.locked ? (
        <Banner tone="error">
          Locked by a red rejection. The student cannot resubmit until you
          reopen it below, after speaking with them.
        </Banner>
      ) : null}
            ...
            <>
              ...
              <Field label="Year group" htmlFor="year_grade">
                <select
                  ...
                  onChange={(event) =>
                    setForm({ ...form, year_grade: event.target.value })
                  }
                >
              ...
              <Field label="Class or roll group" htmlFor="class_roll_group">
                <input
                  ...
                  onChange={(event) =>
                    setForm({ ...form, class_roll_group: event.target.value })
                  }
                />
              </Field>
              <Field label="Costume name" htmlFor="costume_name">
                <input
                  ...
                  onChange={(event) =>
                    setForm({ ...form, costume_name: event.target.value })
                  }
                />
              </Field>
              <Field label="Costume description" htmlFor="costume_description">
                <textarea
                  ...
                  onChange={(event) =>
                    setForm({
                      ...form,
                      costume_description: event.target.value,
                    })
                  }
                />
              </Field>
              ...
              <div className="button-row">
                {submission.previousPhotos.map((photo) => (
                  <figure key={photo.replacedAt} style={{ margin: 0 }}>
                    <img
                      className="photo photo--thumb"
                      src={photo.photoUrl}
                      alt="Previous version"
                    />
                    <figcaption className="small muted">
                      {formatDateTime(photo.replacedAt)}
                    </figcaption>
                  </figure>
                ))}
              </div>
      ...
      <Card title="Review decision">
        ...
        <p className="muted small">
          Rejecting sends the student two things: your written message and the
          reason you pick below. Both also appear in the audit log. Yellow
          reasons let the student fix the problem and resubmit on their own. Red
          reasons lock the entry — the student is told to speak to a teacher,
          and an Admin or the Owner has to reopen it before they can resubmit.
        </p>
        ...
        >
          <select
            ...
            onChange={(event) => {
              ...
              if (chosen && !rejectReason.trim())
                setRejectReason(chosen.suggestedMessage);
            }}
          >
        ...
        {selectedReason?.severity === "serious" ? (
          <Banner tone="error">
            Red reason. The entry will be locked: the student is told to speak
            to a teacher and cannot resubmit until an Admin or the Owner reopens
            it.
          </Banner>
        ) : null}
        ...
        <Field
          ...
          error={rejectErrors.internalNote}
        >
          <textarea
            id="internal-note"
            value={internalNote}
        ...
        <Button
          ...
          onClick={() => {
            ...
            if (!rejectCode)
              problems.code = "Choose a reason for rejecting this entry.";
            if (rejectReason.trim().length < 5) {
              problems.reason = "Write a message the student can act on.";
            }
            if (
              selectedReason?.severity === "serious" &&
              internalNote.trim().length < 5
            ) {
              problems.internalNote =
                "A red reason needs a staff note explaining what happened.";
            }
      ...
      {submission.locked ? (
        <Card title="Reopen entry">
          <p className="muted small">
            Reopen only after speaking with the student. They will be told their
            entry is reopened and that they can resubmit before the deadline,
            following the rules and dress code. The note below is private to
            staff and recorded in the audit log.
          </p>
          ...
          <Button
            ...
            onClick={() =>
              run(async () => {
                await api.post(
                  `/api/admin/submissions/${submission.id}/unlock`,
                  {
        ...
        {canDelete ? (
          <>
            <p className="muted small">
              ...
              be undone. Record why, so the action makes sense in the audit log
              later. Rejecting is almost always the better choice: it keeps the
          ...
          <p className="muted small">
            Only the Owner can delete an entry, so a mistake or a misused Admin
            account cannot erase submissions. Reject the entry instead — use a
            red reason if it is serious — and the Owner can remove it if it
            really has to go.
          </p>
        )}
      </Card>
