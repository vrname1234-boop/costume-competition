...
const CONFIRMATIONS = [
  ...
  { key: 'followsRules', label: 'The image follows the school rules and dress code.' },
  { key: 'clearFullBody', label: 'The image is clear and shows the full costume.' },
  { key: 'understandsDeadline', label: 'I understand submissions close at the deadline shown.' },
] as const;
...
export function EntryForm({ existing }: Props) {
  ...
  const [yearGrade, setYearGrade] = useState(existing?.yearGrade ?? '');
  const [classRollGroup, setClassRollGroup] = useState(existing?.classRollGroup ?? '');
  const [houseId, setHouseId] = useState(existing?.houseId ?? '');
  const [categoryId, setCategoryId] = useState(existing?.categoryId ?? '');
  const [costumeName, setCostumeName] = useState(existing?.costumeName ?? '');
  const [costumeDescription, setCostumeDescription] = useState(existing?.costumeDescription ?? '');
  ...
  const accept = site?.competition.allowedFileTypes.join(',') ?? 'image/jpeg,image/png,image/webp';
  ...
  const submit = (event: FormEvent) => {
    ...
    const work = async () => {
      if (existing) {
        await api.patch<{ submission: StudentSubmission }>('/api/me/submission', details);
        if (file) {
          const form = new FormData();
          form.append('photo', file);
          await api.upload<{ submission: StudentSubmission }>('/api/me/submission/photo', form, 'PUT');
        }
      } else {
        const form = new FormData();
        form.append('details', JSON.stringify({ ...details, confirmations }));
        form.append('photo', file as File);
        await api.upload<{ submission: StudentSubmission }>('/api/me/submission', form);
      }
    };
      ...
      <Card title="Your details">
        <form onSubmit={submit} noValidate>
          ...
          <div className="grid grid--two">
            <Field label="Year group" htmlFor="yearGrade" error={fieldErrors.yearGrade}>
              <select
                id="yearGrade"
                value={yearGrade}
          ...
          <div className="grid grid--two">
            {site && site.houses.length > 0 && (
              <Field label="House (optional)" htmlFor="house">
                <select id="house" value={houseId} onChange={(event) => setHouseId(event.target.value)}>
          ...
          <Field label="Costume name" htmlFor="costumeName" error={fieldErrors.costumeName}>
            <input
              id="costumeName"
              value={costumeName}
          ...
          {!existing && (
            <fieldset style={{ border: 0, padding: 0, margin: '0 0 1rem' }}>
              <legend className="field__label">Before submitting, please confirm</legend>
              {CONFIRMATIONS.map((item) => (
                <label className="checkbox" key={item.key}>
                  <input
                    ...
                    onChange={(event) =>
                      setConfirmations((prev) => ({ ...prev, [item.key]: event.target.checked }))
                    }
                  />
          ...
          <div className="button-row">
            ...
            <Button variant="secondary" onClick={() => navigate('/dashboard')} disabled={busy}>
              Cancel
            </Button>
          </div>
        </form>
